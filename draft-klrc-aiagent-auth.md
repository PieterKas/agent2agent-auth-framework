...
Agents act on behalf of a user, a system, or on their own behalf as shown in {{fig-ai-agent-workload}} and need to obtain authorization when interacting with protected resources. In order to obtain authorization, an Agent may be required to authenticate itself using the mechanisms described in {{agent_authentication}}. 
...
When a User delegates authorization to an Agent, the Agent SHOULD obtain an access token using the Authorization Code Grant as described in {{Section 4.1 of OAUTH-FRAMEWORK}}. This redirection-based flow involves an interactive authorization process in which the user authenticates to the authorization server and explicitly approves the requested access. Users SHOULD authenticate using phishing-resistant authentication mechanisms such as a passkey. The resulting access token reflects the authorization delegated to the Agent by the User and can be used by the Agent to access resources on behalf of the user. The use of OAuth negates the need for the Agent to have access to a User's credentials when accessing a resource on the User's behalf. It is RECOMMENDED that an Agent use the credentials described in {{}} to authenticate to the OAuth Authorization Server when requesting an Access Token. When using the Authorization Code Grant, the Agent authenticates itself using one of the mechanisms described in {{agent_authentication}} and not with the use of static, long-lived client secrets. When using the JWT Authorization Grant, the Agent will be identified in the subject of the JWT assertion.

### Agent Obtains Own Authorization {#agent_obtains_own_access_token}
Agents obtaining access tokens on their own behalf can use the Client Credentials Grant as described in {{Section 4.4 of OAUTH-FRAMEWORK}} or the JWT Authorization Grant as described in {{Section 2.1 of !OAUTH-CLIENTAUTH-JWT=RFC7523}}. When using the Client Credentials Grant, the Agent authenticates itself using one of the mechanisms described in {{agent_authentication}} and not with the use of static, long-lived client secrets. When using the JWT Authorization Grant, the Agent will be identified in the subject of the JWT assertion.
...
## Risk Reduction with Transaction Tokens {#txn-tokens-risk-reduction}
...
Tools expose interfaces to underlying services and resources. Access to the Tools can be controlled by OAuth and augmented by policy, attribute or role based authorization systems (amongst others). If the Tools are implemented as one or more microservices, it should use transaction tokens to reduce risk as described in {{txn-tokens-risk-reduction}} to avoid passing access tokens around within the Tool implementation.
