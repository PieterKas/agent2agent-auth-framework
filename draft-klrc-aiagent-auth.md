...
Agents obtaining access tokens on their own behalf can use the Client Credentials Grant as described in {{Section 4.4 of OAUTH-FRAMEWORK}} or the JWT Authorization Grant as described in {{Section 2.1 of OAUTH-CLIENTAUTH-JWT}}. When using the Client Credentials Grant, the Agent authenticates itself using the credentials described in {{agent_credentials}} with a compatible OAuth client authentication mechanism listed in {{user_delegates_authorization}}, and not with the use of static, long-lived client secrets. When using the JWT Authorization Grant, the Agent will be identified in the subject of the JWT assertion.
